library(ggplot2)
library(reshape2)
file <- "trial-mtu-2048-random.csv"
...
ggplot(melt(frame), aes(x = value, fill = variable)) + geom_density(alpha=0.5) + xlim(4000, 5000) + labs(title=file, x="cycles", fill="read type")
